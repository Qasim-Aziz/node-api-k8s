export const getNextMessageQuery = () => `
  ...
  FROM message m
  LEFT JOIN viewing v ON v.message_id = m.id AND v.deleted_at is NULL AND v.patient_id = :reqPatientId
  WHERE m.deleted_at IS NULL and m.patient_id <> :reqPatientId and m.privacy = 'PUBLIC'::enum_message_privacy
  GROUP BY m.id
