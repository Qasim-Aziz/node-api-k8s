...
describe('# Message Tests', () => {
  ...
  test('should update an existing message only if mine', () =>
    Testers.updateMessage(user2, message2.id, message2Update, { nbLikes: 0, nbViews: 0 }));
  ...
  test('should get a private message if mine and not update viewing count', () =>
    Testers.getMessage(user1, message1.id, { nbViews: 0, nbLikes: 0 }));
  test('should get a public message even if not mine', () =>
    Testers.getMessage(user1, message2.id, { nbLikes: 0, nbViews: 1 }));
  test('shouldnt be able to like a private message', () =>
    Testers.likeMessage(user2, message1.id, { status: httpStatus.BAD_REQUEST }));
  test('should like a public message and update nbLikes', () =>
    Testers.likeMessage(user1, message2.id, { nbLikes: 1, nbViews: 1 }));
  test('should unlike a public message', () =>
    Testers.likeMessage(user1, message2.id, { nbLikes: 0, nbViews: 1 }));
  test('should get the right next messages', async () => {
    //I don't use Promise all here because the time creation is important
    ...
    await Testers.getMessage(user3, message4.id, { nbLikes: 0, nbViews: 1 });
