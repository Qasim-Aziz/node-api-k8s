            ...
            .then(([results]) => {
              ...
              const set = {
                lastRun: migrations.sort(
                  (a, b) => ((a.timestamp < b.timestamp || a.title < b.title) ? 1 : -1))[0],
                migrations,
              };
              fn(null, set);
            })
        ...
        save: function (list, fn) {
          ...
          sequelize.query(
            `INSERT INTO "SequelizeMeta" VALUES ${migrations.map(
              (m, index) => `($${index * 2 + 1}, $${index * 2 + 2})`).join(',')} ON CONFLICT DO NOTHING;`,
            { bind: migrations.reduce(
              (a, m) => a.concat([m.title, new Date(m.timestamp)]), []), transaction: trans },
          )
            .then(() => fn())
            .catch(fn);
        },
