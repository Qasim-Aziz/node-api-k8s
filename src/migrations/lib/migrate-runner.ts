...
export async function runMigrations({ transaction = null } = {}) {
  ...
  await transact(async (trans) => {
      ...
      stateStore: {
        load: function(fn) {
          sequelize.query('CREATE TABLE IF NOT EXISTS "SequelizeMeta" (name varchar(255) PRIMARY KEY);', {transaction: trans })
            ...
            .then(() => sequelize.query('ALTER TABLE "SequelizeMeta" ADD COLUMN IF NOT EXISTS "timestamp" timestamp with time zone;', {transaction: trans }))
            ...
            .then(([results]) => {
              ...
              const set = {
                lastRun: migrations.sort((a,b) => (a.timestamp < b.timestamp || a.title < b.title) ? 1 : -1)[0],
                migrations
              };
              fn(null, set);
            })
        ...
        save: function(set, fn) {
          const migrations = set.migrations.filter(mig => !!mig.timestamp);
          for(const migration of migrations) {
            migration.title
            migration.timestamp
          }
          sequelize.query(
            `INSERT INTO "SequelizeMeta" VALUES ${migrations.map((m, index) => `($${index * 2 + 1}, $${index * 2 + 2})`).join(',')} ON CONFLICT DO NOTHING;`,
            { bind: migrations.reduce((a, m) => a.concat([m.title, new Date(m.timestamp)]), []), transaction: trans })
            .then(() => fn())
            .catch(fn);
        }
      },
    ...
    }, (err, set) => err ? reject(err) : resolve(set)));
